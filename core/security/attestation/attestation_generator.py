...
class Metadata:
    ...
    completeness: dict[str, bool] = field(
        default_factory=lambda: {"parameters": True, "environment": True, "materials": True}
    )
...
class CerteusSigner:
    ...
    def sign(self, payload: bytes) -> dict[str, str]:
        ...
        return {"keyid": self.key_id, "sig": base64.b64encode(signature).decode('utf-8')}
...
class CosignSigner:
    ...
    def sign_container(self, image_ref: str, key_path: str | None = None, attestation_path: str | None = None) -> bool:
    ...
    def verify_container(self, image_ref: str, public_key_path: str) -> bool:
        ...
        try:
            cmd = [self.cosign_path, "verify", "--key", public_key_path, image_ref]
...
class AttestationGenerator:
    ...
    def generate_slsa_provenance(
        ...
        # Create builder info
        builder = Builder(id=self.builder_id, version={"certeus-engine": "1.0.0"})

        # Build configuration
        invocation = BuildConfig(
            commands=build_commands,
            environment=build_env,
            source={"repository": "https://github.com/certeus/engine", "ref": "main"},
        )
        ...
        return SLSAProvenance(
            builder=builder, buildType=self.build_type, invocation=invocation, metadata=metadata, materials=materials
        )

    def create_in_toto_statement(
        self, subjects: list[Subject], predicate_type: PredicateType, predicate: dict[str, Any]
    ) -> InTotoStatement:
        """Create in-toto attestation statement"""

        return InTotoStatement(subject=subjects, predicateType=predicate_type.value, predicate=predicate)

    def sign_statement(self, statement: InTotoStatement, key_id: str) -> DSSEEnvelope:
        ...
        return DSSEEnvelope(payload=payload_b64, signatures=[signature_info])
        ...
        try:
            ...
            for sig_info in envelope.signatures:
                if sig_info.get("keyid") == key_id:
                    ...
                    is_valid = self.key_manager.verify_signature(data=pae, signature=signature, key_id=key_id)
    ...
    def create_build_attestation(
        self, artifact_path: Path, build_commands: list[str], key_id: str, slsa_level: SLSALevel = SLSALevel.LEVEL_2
    ) -> DSSEEnvelope:
        ...
        subject = Subject(name=artifact_path.name, digest={"sha256": artifact_digest})
        ...
        statement = self.create_in_toto_statement(
            subjects=[subject], predicate_type=PredicateType.SLSA_PROVENANCE_V1, predicate=asdict(provenance)
        )
    ...
    def create_container_attestation(
        self, image_ref: str, dockerfile_path: Path, key_id: str, cosign_signer: CosignSigner | None = None
    ) -> DSSEEnvelope | None:
        """Create attestation for container image"""

        # Create SLSA provenance for container build
        build_commands = [f"docker build -f {dockerfile_path} .", f"docker tag {image_ref}"]
        ...
        provenance = self.generate_slsa_provenance(
            subjects=[subject], build_commands=build_commands, build_env={"DOCKER_BUILDKIT": "1"}, materials=materials
        )
    ...
    def verify_attestation_chain(self, attestations: list[DSSEEnvelope], trusted_keys: list[str]) -> bool:
