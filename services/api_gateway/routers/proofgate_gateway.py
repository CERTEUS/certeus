...
def publish_stub(req: PublishRequest, request: Request) -> PublishResponse:  # pragma: no cover (kontrakt/openapi)
    ...
    # Best-effort billing: allocate cost units if provided (no-fail)
    try:
        if isinstance(req.budget_tokens, int) and req.budget_tokens > 0:
            from services.api_gateway.limits import enforce_limits

            enforce_limits(request, cost_units=int(req.budget_tokens))
    except Exception:
        pass
