...
class CerteusClient:
    ...
    def devices_qoracle_expectation(self, *, question: Optional[str] = None, constraints: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def devices_chronosync_reconcile(self, coords: Dict[str, Any], pc_delta: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def packs_install(self, pack: str, signature: str, version: Optional[str] = None) -> Dict[str, Any]:
    ...
    # --- Billing ---
    def billing_quota(self, tenant: Optional[str] = None) -> Dict[str, Any]:
        # Tenant selected via headers (X-Tenant-ID); optional param for helper
        r = self.get("/v1/billing/quota")
        r.raise_for_status()
        return r.json()

    def billing_set_quota(self, units: int, tenant: Optional[str] = None) -> Dict[str, Any]:
    ...
    # --- FIN tokens demo ---
    def fin_tokens_request(self, user_id: str, amount: int, purpose: Optional[str] = None) -> Dict[str, Any]:
        r = self.post("/v1/fin/tokens/request", {"user_id": user_id, "amount": amount, "purpose": purpose})
        r.raise_for_status()
        return r.json()

    def fin_tokens_allocate(self, request_id: str, allocated_by: Optional[str] = None) -> Dict[str, Any]:
