...
class QTMPService:
    ...
    async def _add_type_b_component(self, budget: UncertaintyBudget, spec: dict[str, Any]):
        ...
        distribution = distribution_map.get(spec.get('distribution', 'uniform'), DistributionType.UNIFORM)
    ...
    async def verify_measurement(self, measurement: MeasurementResult) -> dict[str, Any]:
        ...
        metadata_complete = all(
            [measurement.metadata.operator_id, measurement.metadata.procedure_id, measurement.metadata.instrument_id]
        )
    ...
    async def convert_measurement_unit(self, measurement_id: str, target_unit: str) -> MeasurementResult | None:
    ...
    async def batch_create_measurements(self, measurements_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
        ...
        for i, data in enumerate(measurements_data):
            try:
                ...
                results.append({'index': i, 'success': False, 'error': str(e), 'data': data})
                logger.error(f"Batch measurement creation failed at index {i}: {e}")

        logger.info(
            f"Batch created {len([r for r in results if r['success']])} out of {len(measurements_data)} measurements"
        )
        return results
