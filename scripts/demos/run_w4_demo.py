...
def main() -> int:
    ...
    c.post("/v1/qtm/decoherence", json={"case": case, "channel": "dephasing", "gamma": 0.2})
    # Run sequence L->T
    r_lt = c.post("/v1/qtm/measure_sequence", json={"operators": ["L", "T"], "case": case})
    # Run sequence T->L
    r_tl = c.post("/v1/qtm/measure_sequence", json={"operators": ["T", "L"], "case": case})
    lt_p = [float(s.get("p", 0.0)) for s in (r_lt.json().get("steps", []) if r_lt.status_code == 200 else [])]
    tl_p = [float(s.get("p", 0.0)) for s in (r_tl.json().get("steps", []) if r_tl.status_code == 200 else [])]
    ...
    rep = {
        ...
        "tl_probs": tl_p,
        "diff_last": d,
        "uncertainty_lt": (r_lt.json().get("uncertainty_bound", {}).get("L_T") if r_lt.status_code == 200 else None),
        "uncertainty_tl": (r_tl.json().get("uncertainty_bound", {}).get("L_T") if r_tl.status_code == 200 else None),
    }
    Path("reports").mkdir(parents=True, exist_ok=True)
    Path("reports/w4_demo.json").write_text(json.dumps(rep, indent=2), encoding="utf-8")
    print(json.dumps({"ok": r_lt.ok and r_tl.ok if hasattr(r_lt, "ok") else True, "diff_last": d}))
    return 0
