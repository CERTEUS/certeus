...
class NISTNVDClient:
    ...
    def __init__(self, api_key: str | None = None, rate_limit: float = 0.6):
        ...
        # Headers
        self.headers = {"User-Agent": "CERTEUS-CVE-Scanner/1.0", "Accept": "application/json"}
        if api_key:
        ...
        for vuln_data in vulnerabilities:
            try:
                ...
                for desc in cve_data.get("descriptions", []):
                    descriptions.append({"lang": desc.get("lang", "en"), "value": desc.get("value", "")})

                # References
                references = []
                for ref in cve_data.get("references", []):
                    references.append(
                        CVEReference(url=ref.get("url", ""), source=ref.get("source", ""), tags=ref.get("tags", []))
                    )
...
class CVEDatabase:
    ...
    def search_cves_by_severity(self, min_severity: CVESeverity) -> list[CVEItem]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """.format(','.join('?' * len(severity_filter))),
                severity_filter,
            )
...
class CVEScanner:
    ...
    def __init__(self, db_path: Path | None = None, nvd_api_key: str | None = None, cache_duration_hours: int = 24):
    ...
    async def scan_components(self, components: list[Component]) -> dict[str, list[Vulnerability]]:
        ...
        python_packages = [c for c in components if c.type == ComponentType.LIBRARY and c.group == "python"]
    ...
    async def update_cve_database(self, days_back: int = 7):
        ...
        async with NISTNVDClient(api_key=self.nvd_api_key) as nvd_client:
            cves = await nvd_client.search_cves(
                last_mod_start_date=start_date, last_mod_end_date=end_date, results_per_page=2000
            )
    ...
    def generate_vulnerability_report(self, scan_results: dict[str, list[Vulnerability]]) -> dict[str, Any]:
    ...
    def _generate_recommendations(self, scan_results: dict[str, list[Vulnerability]]) -> list[str]:
