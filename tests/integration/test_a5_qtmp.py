...
class TestQTMPService:
    ...
    async def test_measurement_creation_via_service(self):
        ...
        measurement = await service.create_measurement(
            value=125.67, unit="g", uncertainty_components=uncertainty_components, metadata=metadata
        )
...
class TestDoD:
    ...
    async def test_dod_e2e_csv_to_verification(self):
        ...
        for row in csv_data:
            measurement = create_simple_measurement(
                value=row['value'], unit=row['unit'], uncertainty=row['uncertainty'], operator_id=row['operator']
            )
    ...
    async def test_dod_measurement_repeatability(self):
        ...
        for _i in range(5):
            measurement = create_simple_measurement(
                value=10.0, unit="m", uncertainty=0.1, operator_id="test_operator", procedure_id="repeatability_test"
            )
            measurements.append(measurement)
