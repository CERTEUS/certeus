...
async def stress_test_concurrent():
    ...
    for i in range(100):
        ...
        await ledger.create_case(
            case_id=case_id,
            jurisdiction={'test': True, 'concurrent': i}
        )
        ...
        for j in range(5):
            task = ledger.record_event(
                ...
                payload={'test': i, 'event': j, 'concurrent': True}
            )
            tasks.append(task)
    ...
    async with ledger.get_connection() as conn:
        ...
        chain_positions = await conn.fetch('SELECT chain_position, COUNT(*) as cnt FROM events GROUP BY chain_position HAVING COUNT(*) > 1 ORDER BY chain_position LIMIT 5')
