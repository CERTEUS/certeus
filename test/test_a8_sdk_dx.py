...
class TestA8SDKGenerator(unittest.TestCase):
    ...
    def test_sdk_generator_script_exists(self):
        ...
        self.assertTrue(self.sdk_generator_path.exists(), 
                       f"SDK generator script not found: {self.sdk_generator_path}")
...
class TestA8ContractTester(unittest.TestCase):
    ...
    def test_contract_tester_script_exists(self):
        ...
        self.assertTrue(self.contract_tester_path.exists(),
                       f"Contract tester script not found: {self.contract_tester_path}")
...
class TestA8DXEnhancer(unittest.TestCase):
    ...
    def test_dx_enhancer_script_exists(self):
        ...
        self.assertTrue(self.dx_enhancer_path.exists(),
                       f"DX enhancer script not found: {self.dx_enhancer_path}")
...
class TestA8SDKStructure(unittest.TestCase):
    ...
    def test_existing_typescript_sdk_structure(self):
        ...
        ts_sdk_dirs = [
            REPO_ROOT / "sdk" / "ts" / "src",
            REPO_ROOT / "clients" / "typescript" / "certeus-sdk" / "src"
        ]
...
class TestA8ScriptExecution(unittest.TestCase):
    ...
    def test_sdk_generator_script_syntax(self):
        ...
        if script_path.exists():
            ...
            self.assertEqual(result.returncode, 0,
                           f"SDK generator syntax error: {result.stderr}")
    ...
    def test_contract_tester_script_syntax(self):
        ...
        if script_path.exists():
            ...
            self.assertEqual(result.returncode, 0,
                           f"Contract tester syntax error: {result.stderr}")
    ...
    def test_dx_enhancer_script_syntax(self):
        ...
        if script_path.exists():
            ...
            self.assertEqual(result.returncode, 0,
                           f"DX enhancer syntax error: {result.stderr}")
    ...
    def test_a8_scripts_have_proper_headers(self):
        ...
        if scripts_dir.exists():
            for script_file in scripts_dir.glob("*.py"):
                ...
                self.assertIn("CERTEUS", content,
                            f"Missing CERTEUS header in {script_file}")
                self.assertIn("FILE:", content,
                            f"Missing FILE declaration in {script_file}")
                self.assertIn("ROLE:", content,
                            f"Missing ROLE declaration in {script_file}")
        ...
        if scripts_dir.exists():
            for script_file in scripts_dir.glob("*.py"):
                ...
                self.assertIn('"""', content,
                            f"Missing docstring in {script_file}")
                # Check for bilingual documentation
                has_pl = "PL:" in content
                has_en = "EN:" in content
                self.assertTrue(has_pl or has_en,
                              f"Missing bilingual docs in {script_file}")
        ...
        if scripts_dir.exists():
            for script_file in scripts_dir.glob("*.py"):
                ...
                if script_file.name != "__init__.py":
                    self.assertTrue(has_retry,
                                  f"No retry logic found in {script_file}")
            ...
            for script_file in scripts_dir.glob("*.py"):
                ...
                if script_file.name != "__init__.py":
                    self.assertTrue(has_error_handling,
                                  f"No error handling found in {script_file}")
    ...
    def test_a8_typescript_compatibility(self):
        ...
        for ts_file in ts_files:
            ...
            if len(content.strip()) > 100:  # Skip very small files
                self.assertTrue(has_types or has_exports,
                              f"TypeScript file {ts_file} missing types/exports")
...
def main() -> int:
    ...
    if result.failures:
        ...
        for test, traceback in result.failures:
            print(f"  - {test}: {traceback.split('AssertionError:')[-1].strip() if 'AssertionError:' in traceback else 'Failed'}")
