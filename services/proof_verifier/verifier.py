...
def verify_lfsc(text: str) -> VerificationResult:
    ...
    if mock == "lfsc_fail":
        return VerificationResult(
            ok=False,
            proof_hash=_sha256_hex(data),
            details={"verifier": "mock:lfsc-fail"},
        )
    ...
    if cmd:
        try:
            ...
            return VerificationResult(
                ok=ok,
                proof_hash=_sha256_hex(data),
                details={"verifier": "external:cvc5", "rc": proc.returncode},
            )
            ...
            return VerificationResult(
                ok=False,
                proof_hash=_sha256_hex(data),
                details={"verifier": f"external:cvc5:error:{e}"},
            )
...
def verify_drat(text: str) -> VerificationResult:
    ...
    if mock == "drat_fail":
        return VerificationResult(
            ok=False,
            proof_hash=_sha256_hex(data),
            details={"verifier": "mock:drat-fail"},
        )
    ...
    if cmd:
        try:
            ...
            return VerificationResult(
                ok=ok,
                proof_hash=_sha256_hex(data),
                details={"verifier": "external:drat", "rc": proc.returncode},
            )
            ...
            return VerificationResult(
                ok=False,
                proof_hash=_sha256_hex(data),
                details={"verifier": f"external:drat:error:{e}"},
            )
