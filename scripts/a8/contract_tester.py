...
class ContractTester:
    ...
    def run_schemathesis_tests(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(['python', '-c', 'import schemathesis'], capture_output=True, text=True)

            if result.returncode != 0:
                self.result.warnings.append("Schemathesis not available, installing...")
                # Try to install schemathesis
                install_result = subprocess.run(
                    ['python', '-m', 'pip', 'install', 'schemathesis'], capture_output=True, text=True
                )
            ...
            for failure in failures:
                if failure.get('check') == 'not_a_server_error':
                    ...
                    self.result.errors.append(
                        f"Schema validation failed on {failure.get('method')} {failure.get('path')}"
                    )
...
def main() -> int:
    ...
    # Determine exit code
    success = steps_passed == total_steps and tester.result.failed_tests == 0 and not tester.result.errors
