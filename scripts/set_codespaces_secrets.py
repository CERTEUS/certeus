...
def _api(token: str, method: str, url: str, data: dict | None = None) -> dict:
    ...
    try:
        ...
        raise SystemExit(
            f"API {method} {url} failed: {e.code} {e.reason} -> {msg}") from e
...
def _ensure_pynacl():
    try:
        ...
        subprocess.run([_sys.executable, "-m", "pip",
                       "install", "PyNaCl"], check=True)


def _encrypt(key_b64: str, value: str) -> str:
    from nacl import encoding, public

    pk = public.PublicKey(base64.b64decode(key_b64),
...
def main() -> int:
    ...
    ap.add_argument("--repo", default="CERTEUS/certeus",
                    help="owner/repo to grant access to secrets")
    ap.add_argument("--set", nargs="*",
                    default=["ADMIN_TOKEN", "GITHUB_USER"], help="Secret names to set")
    args = ap.parse_args()

    token = _env_or_file("ADMIN_TOKEN", "GITHUB_PUSH_TOKEN",
                         "GITHUB_TOKEN", "GH_TOKEN")
    if not token:
        raise SystemExit(
            "Missing admin token in env or .devkeys/admin_token.txt")
    ...
    repo_meta = _api(
        token, "GET", f"https://api.github.com/repos/{owner}/{repo}")
