...
class LinuxFUSEFilesystem(PFSFilesystemInterface):
    ...
    async def mount(self, mount_point: str) -> None:
        ...
        self._fuse_thread = threading.Thread(
            target=self._run_fuse_mount, args=(mount_point, fuse_options), name="FUSEMount"
        )
    ...
    def _run_fuse_mount(self, mount_point: str, options: dict[str, Any]) -> None:
        ...
        try:
            ...
            # Create and run FUSE
            FUSE(self._fuse_operations, mount_point, **options)
    ...
    async def unmount(self) -> None:
        ...
        try:
            ...
            result = subprocess.run(['fusermount', '-u', self._mount_point], capture_output=True, text=True, timeout=10)

            if result.returncode != 0:
                self.logger.error(f"fusermount failed: {result.stderr}")

                # Try alternative unmount
                result = subprocess.run(['umount', self._mount_point], capture_output=True, text=True, timeout=10)
    ...
    def _create_demo_tree(self) -> None:
        ...
        demo_file = VirtualFileInfo(
            ...
            extended_attrs=ExtendedAttributes(
                pfs_hash="a" * 64, pfs_size=100, pfs_case_id="DEMO", pfs_mime_type="text/plain"
            ),
        )
...
__all__ = ["LinuxFUSEFilesystem", "LinuxFUSEOperations", "is_fuse_available", "get_fuse_version"]
