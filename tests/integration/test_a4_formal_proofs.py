...
class TestProofArtifactManager:
    ...
    @pytest.mark.asyncio
    async def test_artifact_storage_and_retrieval(
        self, temp_storage
...
class TestZ3Integration:
    ...
    async def test_z3_simulation_fallback(self, default_config):
        ...
        config = SolverConfig(solver_type=SolverType.Z3, fallback_to_simulate=True, random_seed=42)
...
class TestCVC5Integration:
    ...
    async def test_cvc5_satisfiable_formula(self, default_config):
        ...
        config = SolverConfig(
            solver_type=SolverType.CVC5, random_seed=42, generate_artifacts=True, artifact_format="lfsc"
        )
    ...
    async def test_cvc5_simulation_fallback(self, default_config):
        ...
        config = SolverConfig(solver_type=SolverType.CVC5, fallback_to_simulate=True, random_seed=100)
...
class TestFormalProofService:
    ...
    async def test_deterministic_execution(self, proof_service):
        ...
        config = SolverConfig(solver_type=SolverType.Z3, random_seed=12345, fallback_to_simulate=True)

        artifact1 = await proof_service.prove_formula(smt_formula, "det_test_001", config_override=config)

        artifact2 = await proof_service.prove_formula(smt_formula, "det_test_002", config_override=config)
    ...
    async def test_timeout_and_limits_enforcement(self, proof_service):
        ...
        artifact = await proof_service.prove_formula(complex_formula, "timeout_test", config_override=config)
...
class TestDoD:
    ...
    async def test_dod_counterexample_coverage(self, proof_service):
        ...
        if unsat_artifact.result == ProofResult.UNSATISFIABLE:
            ...
            assert (
                unsat_artifact.proof_data is not None
                or unsat_artifact.solver_type in [SolverType.Z3, SolverType.CVC5, SolverType.SIMULATE]
                or "simulated" in unsat_artifact.solver_version
            )
    ...
    async def test_dod_deterministic_seeds(self, proof_service):
        ...
        results = []
        for i in range(3):
            artifact = await proof_service.prove_formula(formula, f"dod_seed_{i}", config_override=config)
    ...
    async def test_dod_simulation_fallback(self, proof_service):
        ...
        simulate_config = SolverConfig(solver_type=SolverType.SIMULATE, fallback_to_simulate=True, random_seed=98765)

        formula = "(declare-fun sim_var () Int) (assert (= sim_var 42))"

        artifact = await proof_service.prove_formula(formula, "dod_simulation", config_override=simulate_config)

        # Simulation should always work
        assert artifact.result in [ProofResult.SATISFIABLE, ProofResult.UNSATISFIABLE, ProofResult.SIMULATED]
        assert artifact.execution_time_ms > 0
