...
hex_chars = st.text(
    alphabet=st.sampled_from(list(string.hexdigits.lower())), min_size=64, max_size=64
)
...
def test_pnip_strict_rejects_invalid_hash_when_enabled(
    bad_hash: str, monkeypatch
) -> None:
    from services.api_gateway.main import app
