...
class SecurityService:
    ...
    def _log_audit_event(self, 
        ...
        if self.policy.audit_logging:
            ...
            with open(audit_file, 'a') as f:
                f.write(f"{event.timestamp.isoformat()} | {severity} | {component} | {event_type} | {json.dumps(details)}\n")
    ...
    async def generate_project_sbom(self, 
        ...
        try:
            ...
            sbom = self.sbom_generator.generate_sbom(
                include_dev_dependencies=self.policy.include_dev_dependencies,
    ...
    async def sign_container_image(self,
        ...
        try:
            ...
            self._log_audit_event(
                "CONTAINER_SIGN_ERROR",
                "CosignSigner",
                {"image_ref": image_ref, "error": str(e)},
                "ERROR"
            )
            return False
    ...
    async def generate_security_report(self) -> SecurityReport:
        ...
        try:
            ...
            security_score = self._calculate_security_score(
                keys_due_rotation, sbom_valid, scan_report
            )
