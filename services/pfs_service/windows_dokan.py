...
class WindowsDokanOperations:
    ...
    def set_file_time(
        self, filename: str, creation_time: float, last_access_time: float, last_write_time: float
    ) -> int:
    ...
    def get_volume_information(self) -> dict[str, Any]:
        ...
        return {
            ...
            "filesystem_flags": (
                dokan.FILE_CASE_PRESERVED_NAMES | dokan.FILE_CASE_SENSITIVE_SEARCH | dokan.FILE_READ_ONLY_VOLUME
            )
            if DOKAN_AVAILABLE
            else 0,
    ...
    def get_disk_free_space(self) -> dict[str, int]:
        ...
        return {"free_bytes_available": 0, "total_number_of_bytes": 0, "total_number_of_free_bytes": 0}
...
class WindowsDokanFilesystem(PFSFilesystemInterface):
    ...
    async def mount(self, mount_point: str) -> None:
        ...
        if mount_point.endswith(":"):
            ...
            self.logger.warning(
                f"Mount point {mount_point} is not a drive letter, using default {self._dokan_config.drive_letter}"
            )
        ...
        self._mount_thread = threading.Thread(target=self._run_dokan_mount, name="DokanMount")
