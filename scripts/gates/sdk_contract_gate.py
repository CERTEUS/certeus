...
def _has_go_sdk(repo: Path) -> bool:
    ...
    try:
        if g.exists():
            ...
            return ("func (c *Client) PFSList" in s) and (
                "func (c *Client) PFSXattrs" in s
            )
    except Exception:
        return False
    return False
...
def main() -> int:
    ...
    has_publish = any(
        p.endswith("/v1/proofgate/publish") and m == "post"
        for (_id, (m, p)) in ops.items()
    ) or ("publish_proofgate_publish" in ops or "proofgate_publish" in ops)
    ...
    try:
        ts_src = (
            (repo / "sdk" / "ts" / "src" / "client.ts").read_text(encoding="utf-8")
            if ts_ok
            else ""
        )
    ...
    try:
        go_src = (
            (repo / "sdk" / "go" / "certeus" / "certeus.go").read_text(encoding="utf-8")
            if go_ok
            else ""
        )
    ...
    (out / "sdk_contract_report.json").write_text(
